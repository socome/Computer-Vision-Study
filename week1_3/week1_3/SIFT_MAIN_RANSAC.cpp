...
int main3()
{
	...
	//찾고자하는 물체(사진)을 SIFT를 통한 특징점 추출&계산
	Detector->detect(matGrayImage, vtKeypointsImage);
	Extractor->compute(matGrayImage, vtKeypointsImage, matDescriptorsImage);
	...
	for (;;)
	{
		...
		//실시간 비디오 프레임을 SIFT를 통한 특징점 추출&계산
		Detector->detect(matGrayvideo, vtKeypointVideo);
		Extractor->compute(matGrayvideo, vtKeypointVideo, matDescriptorsVideo);
		...
		imshow("sift matches", matGoodMatcges);
